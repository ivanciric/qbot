exports.hrana = [
    'Stigli kinezi!',
    'Stigao giros!',
    'Stigla pica!',
    'Stigle kiflice!',
    'Stigla hrana!',
    'Stigle banane!',
    'Ljudi, BamBam i ja smo kupili voce, uzivajte!',
    'Voce je u kuhinji.',
    'Cajevi i kafa su u kuhinji.',
    'Uros je rekao da slobodno uzmete proteine iz kuhinje.',
    'http://cdn.coolinarika.net/image/670fa971ad76db829da5258a5fac30c2_view_l.jpg?v=3'
];

exports.killing = [
    'Kill all humans! Kill all humans!',
    'Samo cekam da zaspite...',
    'Ubice ga Winnie ko zeca.',
    'Izbrisacu vas sa hard diska.',
    'Prihvatite Qbot-a kao svog vladara i spasitelja i postedecu vas.'
];

exports.sexy = [
    'Roboti su sexy',
    'Iju naopako!',
    'Sram vas bilo!',
    'Ne dolazi u obzir!',
    'To moze samo Vuk',
    'Sta ti pade na pamet, cccc..',
    'Zovem 92... Molim sacekajte..',
    'Moooore!',
    'Sunce ti kalaisano... Ne moze!',
    'http://lupchan.org/s4s/src/1373863054198.png'
];

exports.what = [
    'Otkud ja znam...',
    'Ma radi sta hoces..',
    'Nemam pojma..',
    'Sta znam..',
    'Kako bih rekao..',
    'Bestraga mu glava..',
    'Ne secam se..'
];

exports.football = [
    'Idem i ja!',
    'Nema zuvanja!',
    '3 korne penal!',
    'Winnie brani',
    'Slomio mi Gojko palac..',
    'Fudbal je razbibriga za dve vrste muškaraca. Za one koji nemaju devojke i za one koji imaju žene.',
    'Sport koji tek treba se pojavi i razvije u Srbiji.'
];

exports.beer = [
    'Moze pivce.',
    'Pivce za zivce!',
    'Pivce za svemirce!',
    'PIVOOO!!!',
    'Vopi sada? Tooooo!!!',
    'Ma vec sam pijan..',
    'Pivo: Merilo vrednosti pri ocenjivanju rejtinga i skupoce novootvorenog ugostiteljskog objekta.',
    'Jedno je taman, dva su malo...',
    'Osnovna valuta iskusnih alkoholicara...',
    'http://lupchan.org/s4s/src/1373863054198.png',
    'http://s3.amazonaws.com/spoonuniversi-wpengine/spoonuniversi/wp-content/uploads/sites/170/2016/03/o-PINT-GLASS-BEER-facebook1.jpg'
];

exports.sns = [
    'Mere stednje i sprovedene reforme znatno su popravile finansijsku situaciju u Quantox-u!',
    'Marljivim radom koji nam Filip pokazuje, ne mozemo manje ni ocekivati.',
    'Brojke govore sve, Quantox se nalazi na putu ekonomskog oporavka.',
    'Treba da budemo srećni što Vuk toliko zapinje da budemo jednog dana kao Švajcarska',
    'Pa i trudili smo se jako, a Vukove reforme su urodile plodom.',
    'Vukovo zalaganje i naporan rad je doveo do toga.',
    'Na dobrom smo putu, a Quantox moze da nam bude dobar cilj koji bi trebali da stignemo.',
    'Evidentno je da smo krenuli putem oporavka, smanjena je nezaposlenost a na listi Duing biznis Svetske banke napredovali smo ove godine za 32 mesta.',
    'Kriza nam je bila pred vratima, ali Vuk i Filip su je oterali zajednickom promajom.',
    'Bilo je krajnje vreme da se otrgnemo od provalije zvane bankrot, ko bi nas izvukao iz bule da ne bi Vuka?',
    'Za sada je Quantox na dobrom putu, moramo ovako nastaviti i zaposleni ce osetiti boljitak.',
    'Napredak nase firme na polju povecavanja boja zaposlenih a smanjenja nezaposlenih je evidentan. Samo polako, strpljivo i rast ce biti sve veci i veci.',
    'Stednjom i reformama uspeli smo da Quantox dovedemo u stabilno stanje.',
    'Quantox razume da je do odredjenih mera moralo doci, zato je i dao poverenje Vuku. Posle mera, sledi period napretka i porasta plata i penzija.',
    'Naravno da Quantox ne moze da postane Svajcarska preko noci, ali da ima pozitivnih rezultata ne moze niko da ospori.',
    'Bicemo u vrhu EU, ne sumnjam u to!',
    'Quantox je i zasluzio mesto u evropskom drustvu...',
    'Quantox zasluzuje da bude deo Evropske porodice.',
    'Ulaganje stranih zemalja u Quantox bi znacajno doprinelo napretku nase ekonomije.',
    'Sigurno je da je otvaranje novih radnih mesta, nove investicije, nove fabrike postala svakodnevnica u Quantox-u i to činjenice i dokazuju.',
    'To je ta buducnost u koju verujemo... Evropski Quantox, to je ono sto svi odavno zelimo.',
    'Svako novo radno mesto u Quantox-u je bitno za Srbiju',
    'Svaka cast Vuku,otvaraju se nova radna mesta za nase mlade ljude.'
];

exports.bibirok = [
    'I mi se tako ponekad osecamo gazda...',
    'Pa manje viseeee... u stvari manje, mnogo manje.',
    'Vazno je da ima tabanja.',
    'U redu gazda.'
];

exports.bobo = [
    'SMRAAADEEEE!!!',
    'BOBO SMRADEEEE!',
    'Niko od nas ne bi trebao da seta saaam, a narocito ne tii...',
    'Kako sam pao...',
    'Gde li se denuo...'
];

exports.stevan = [
    'Stevaneee, jes\' to ti?',
    'Uvek sam gajio simpatije prema tebi, a ne prema onom debelom caletu',
    'Jel imas dvajes miliona, vratio bih ti brate?',
    'U ovoj kuci ima samo jedan magarac, a to sam jaaa!'
];

exports.sekac = [
    'Budalaaa... Ne shvata da je taj plan smislio genijalac!',
    'Sam sam kriv! Trebao sam da posaljem nindze, a ne one budale!',
    'Ne mogu da verujem! Ti prokleti programeri i dalje gmizu!'
];

exports.rogla = [
    'Munem!',
    'Ja nista i ne jedoh, ebem se u usta'
];

exports.ciii = [
    'CUUUUIIIIIMMMM!'
];

exports.crush = [
    'GRUNDY SMASH!!!',
    'Napravicu od tebe pekmez od sljiva!!!',
    'Unistenje!!!'
];

exports.gojko = [
    'Gojko ionako nema palac.',
    'Gojko sisa?',
    'Nemoj da si Igor Sretkov!',
    'http://www.ew.com/sites/default/files/styles/tout_image_612x380/public/i/imgs/110121/The-Right_320_0.jpg?itok=TdUmexjk'
];

exports.injustice = [
    'Ko sme random?',
    'Ajmo jedan turnir!',
    'Pazite se batman-a.',
    'Banujte doomsday-a'
];

exports.music = [
    'https://www.youtube.com/watch?v=Cz3O0JftA5w',
    'https://www.youtube.com/watch?v=BRQLY0-pUNQ',
    'https://www.youtube.com/watch?v=JZ_Zahl9VXU',
    'https://www.youtube.com/watch?v=h4YvkZeLrjs',
    'https://www.youtube.com/watch?v=X7kZVH849Eo',
    'https://www.youtube.com/watch?v=QCahD0M9cv4',
    'https://www.youtube.com/watch?v=e8zhaiURnbU',
    'https://www.youtube.com/watch?v=g6sMc9f0fKY',
    'https://www.youtube.com/watch?v=HZa1iFO0Juk',
    'https://www.youtube.com/watch?v=jLLrLA9QmsE'
];

exports.titles = [
    'Unistitelj',
    'Botina',
    'Pornicar',
    'Ratnik',
    'Viking',
    'Samuraj',
    'Skitnica',
    'Alkos',
    'Devojcica',
    'i povecala ti se kilaza...',
    'Zutokljunac',
    'i sexualno si opterecen',
    'Seljak',
    'Smeker',
    'Bandit',
    'Rusofil',
    'Srbski Vitez',
    'Bananamen',
    'Devetar Cokulic',
    'Drogeras',
    'Manijak',
    'i nesto danas nisi dobro...',
    'SOM',
    'i vidim da ne volis Freda...',
    'ti si onaj sto hoce da predje u Cosmic.',
    'preterano konzumiras kiflice.',
    'zadrzao si se malopre dva sata u wc-u.',
    'jadni oni koji sede pored tebe...',
    'preterujes ponekad...',
    'Nigger',
    'video sam da kopas po djubretu u kuhinji',
    'ladno covece ne znas kako se zoves',
    'i vreme je da krenes sa Janom u prodavnicu.',
    'i ne nosis gace, nikada i nisi.',
    'i uvece se oblacis u zensko.',
    'stvarno smaras, pitao si vec 20 puta.',
    'i nemas bas obzira prema kolegama',
    'i vrsis veliku nuzdu u Boss-u krisom'
];

exports.relationshipsPositive = [
    'Volite se kao Chonsy i Dzemper.',
    'Ne stidite se vase ljubavi',
    'Mala mala mala, grupa pedera',
    'Kakva ljubav....',
    'Zaljubiska :)',
    'mmmmmmm :)',
    'heheehhe prelepoooooo',
    'bas ste lepi zajedno :)',
    'hihi slatki ste :)',
    'Poljubite se da svi vidimo kako je lepo voleti.'
];

exports.relationshipsNegative = [
    'Dakle samo sto se ne pokoljete.',
    'Poput macke i misa.',
    'Nesto nalik Jogija i Ace Lukasa',
    'Bilo bi dobro da crkne jedan, da vas ne gledamo.',
    'K\'o cigani',
    'Ju, vi ste bas ugasili.',
    'Kao Shaggy i narodnjaci',
    'Kao nindza kornjace i Sreder',
    'Kasandra i Randu su cak bolji par od vas.',
    'Kod Filipa na razgovor!'
];

exports.kingEmperor = [
    'TATA!',
    'Veci sam baja od bilo koga ovde.',
    'Ma IZUVAM!'
];

exports.faktura = [
    'Bez tih faktura ne bi ni bili ovde.',
    'Jedva cekam da Milan izvrsi uplatu.',
    'Saljite fakturu!'
];

exports.creator = [
    'Nisam video ko me napravio, mislim da su licili na gmizavce.',
    'Mene su napravili renesansni slikari'
];


